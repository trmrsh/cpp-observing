...
*airmass* plots the airmass versus time for a group of stars. Note that the
format of the input coordinate/ephemeris files has changed as I have switched
to ICRS coordinates and so no equinox should be specified.

Invocation: airmass file date telescope device
...
The 'null' just indicates that a star has no ephemeris info.  The HMJD above
says that the timescale is heliocentrically-corrected UTC, expressed as an
MJD. Other choices are HJD (heliocentric UTC as a JD), and BJD and BMJD which
are the barycentric equivalents using TDB (no point barycentrically correcting
UTC as it is not a good timescale). 'linear' refers to the number of terms of
the ephemeris. The alternative is quadratic in which case a third coefficient
